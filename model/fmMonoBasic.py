...
def my_lfilter(coeff, x):
	...
	for i in range(block_len):
		for j in range(coeff_len):
			if ((i-j) >= 0):
				processed_signal[i] += coeff[j]*x[i-j]
	return processsed_signal
...
if __name__ == "__main__":
	...
	if il_vs_th == 0:
		...
		audio_filt = np.array(audio_filt)
	else:
		# to be updated by you for the takehome exercise
		# with your own code for single pass convolution
		audio_filt = np.array(my_lfilter(audio_coeff,fm_demod))
